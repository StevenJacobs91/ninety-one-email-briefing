...
export function BriefSummary({ data }: BriefSummaryProps) {
  ...
  function sectionVisible(fieldId: string): boolean {
    const f = settings.formFields.find((field) => field.id === fieldId)
    return f ? f.visible : true
  }

  return (
    <div className="space-y-6 text-sm">
      {sectionVisible('review.campaignSection') && (
      ...
      {sectionVisible('review.audienceSection') && (
      ...
      {sectionVisible('review.contentSection') && (
      ...
      {sectionVisible('review.assetsSection') && (
      ...
      {sectionVisible('review.deadlinesSection') && (
